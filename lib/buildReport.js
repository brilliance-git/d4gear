...
function buildAllVariantsReport(doc) {
  const variants = doc?.data?.buildVariants?.values || [];
  if (variants.length === 0) {
    throw new Error('No build variants found in this document.');
  }
  const buildName = doc?.data?.name || doc?.slugifiedName || 'Unknown Build';
  return {
    buildSlug: doc?.slugifiedName || doc?.id || slugify(buildName),
    buildName,
    variants: variants.map((_, i) => buildReport(doc, i)),
  };
}

module.exports = { buildReport, buildAllVariantsReport, slugToLabel, slugify };
